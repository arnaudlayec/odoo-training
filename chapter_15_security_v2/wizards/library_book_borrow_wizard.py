...
class LibraryBookBorrowWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "partner_id" in fields_list and not defaults.get("partner_id"):
            # Bonus : un Lecteur emprunte pour lui-même par défaut ; un Bibliothécaire
            # choisit l'emprunteur (il implique aussi group_library_reader, d'où l'ordre du test).
            if self.env.user.has_group("library.group_library_reader") and not self.env.user.has_group(
                "library.group_library_librarian"
            ):
